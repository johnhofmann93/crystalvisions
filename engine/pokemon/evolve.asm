...
EvolveMidBattle:
; Like EvolveAfterBattle but triggered during a trainer battle when a mon
; levels up to its evolution level and the trainer still has mons remaining.
; Skips EVOLVE_TRADE and EVOLVE_ITEM (require deliberate player action).
; Updates the active battle mon's stats and types if the evolved mon is in play.
; Restarts battle BGM after evolution animation.
	xor a
	ld [wMonTriedToEvolve], a
	dec a
	ld [wCurPartyMon], a
...
.skip_unown
	...
	; Sync species, level, stats, and types to the active battle mon struct
	ld a, [wCurPartyMon]
	ld hl, wPartyMons
	ld bc, PARTYMON_STRUCT_LENGTH
	call AddNTimes
